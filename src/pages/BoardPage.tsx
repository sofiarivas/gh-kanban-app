    ...
    <div className="Board">
      ...
      <div className="Board__container">
        <div className="Board__column">
          ...
          {branches.map((branch) => branch.status === 'in-progress' && <Card key={branch.name} name={branch.name} rightOnClick={moveToReview} />)}
        </div>
        <div className="Board__column">
          <p className="Board__subtitle">Review</p>
          {branches.map((branch) => branch.status === 'review' && <Card key={branch.name} name={branch.name} leftOnClick={moveToInProgress} rightOnClick={moveToReadyToMerge} />)}
        </div>
        <div className="Board__column">
          <p className="Board__subtitle">Ready to merge</p>
          {branches.map((branch) => branch.status === 'ready-to-merge' && <Card key={branch.name} name={branch.name} leftOnClick={moveToReview} />)}
        </div>
      </div>
    </div>
  );
